...
static psa_algorithm_t ssl_tls13_get_ciphersuite_hash_alg(int ciphersuite)
{
    ...
    if (ciphersuite_info != NULL) {
        return mbedtls_psa_translate_md((mbedtls_md_type_t)ciphersuite_info->mac);
    }
                                                     ...
                                                     const unsigned char *end)
{
    ...
    if (mbedtls_psa_translate_md((mbedtls_md_type_t)ssl->handshake->ciphersuite_info->mac)
        != hash_alg) {
        MBEDTLS_SSL_DEBUG_MSG(
                                                    ...
                                                    size_t ticket_nonce_len)
{
    ...
    psa_hash_alg = mbedtls_psa_translate_md((mbedtls_md_type_t)ciphersuite_info->mac);
