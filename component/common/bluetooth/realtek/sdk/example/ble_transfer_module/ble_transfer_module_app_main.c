...
extern char dev_name[BLE_TRANSFER_MODULE_DEV_NAME_MAX_LEN];
extern uint16_t bt_stack_le_conn_handle[BLE_TRANSFER_APP_MAX_LINKS];
 ...
 * @return void
 */
void ble_transfer_bt_stack_config_init(void)
{
    gap_config_max_le_link_num(BLE_TRANSFER_APP_MAX_LINKS);
    gap_config_max_le_paired_device(BLE_TRANSFER_APP_MAX_LINKS);
}
...
/**
  ...
  */
extern char dev_name[BLE_TRANSFER_MODULE_DEV_NAME_MAX_LEN];
void ble_transfer_app_le_gap_init(void)
{
    ...
    le_register_app_cb(ble_transfer_app_gap_callback);
#if F_BT_LE_USE_RANDOM_ADDR
		T_APP_STATIC_RANDOM_ADDR random_addr;
		bool gen_addr = true;
		uint8_t local_bd_type = GAP_LOCAL_ADDR_LE_RANDOM;
		if (ble_transfer_app_load_static_random_address(&random_addr) == 0)
		{
		...
		if (gen_addr)
		{
			if (le_gen_rand_addr(GAP_RAND_ADDR_STATIC, random_addr.bd_addr) == GAP_CAUSE_SUCCESS)
			{
				...
				ble_transfer_app_save_static_random_address(&random_addr);
			}
		}
		...
		gatt_register_callback((void*)ble_transfer_gap_service_callback);
#endif
 ...
 * @return void
 */
void ble_transfer_app_le_profile_init(void)
{
    client_init(1);
    ble_transfer_gcs_client_id = gcs_add_client(ble_transfer_gcs_client_callback, BLE_TRANSFER_APP_MAX_LINKS, BLE_TRANSFER_APP_MAX_DISCOV_TABLE_NUM);
 ...
 * @return   void
 */
void ble_transfer_task_init(void)
{
    ble_transfer_app_task_init();
}
 ...
 */
int ble_transfer_app_main(void)
{
	bt_trace_init();
    ble_transfer_bt_stack_config_init();
	bte_init();
	le_gap_init(BLE_TRANSFER_APP_MAX_LINKS);
    ble_transfer_app_le_gap_init();
    ble_transfer_app_le_profile_init();
    ble_transfer_task_init();
    return 0;
}

int ble_transfer_app_init(void)
{
	...
	if (new_state.gap_init_state == GAP_INIT_STATE_STACK_READY) {
		printf("[BLE Transfer]BT Stack already on\r\n");
		return 0;
	}
	else
		ble_transfer_app_main();
...
int ble_transfer_app_deinit(void)
{
	...
	ble_transfer_app_task_deinit();
	T_GAP_DEV_STATE state;
	le_get_gap_param(GAP_PARAM_DEV_STATE , &state);
	if (state.gap_init_state != GAP_INIT_STATE_STACK_READY) {
		printf("[BLE Transfer]BT Stack is not running\r\n");
	}
#if F_BT_DEINIT
	else {
		gcs_delete_client();
		bte_deinit();
		bt_trace_uninit();
		printf("[BLE Transfer]BT Stack deinitalized\r\n");
	}
#endif
	return 0;
}
